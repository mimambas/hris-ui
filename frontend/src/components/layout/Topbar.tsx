...
import { Search, Bell, Menu, X, ArrowUpRight, Users, Wallet, CalendarDays, FileText, Settings, Clock, Sun, Moon, BookOpen } from 'lucide-react';
...
export default function Topbar() {
  ...
  return (
    <header className="h-16 bg-canvas/95 backdrop-blur-sm border-b border-hairline flex items-center justify-between px-5 lg:px-8 sticky top-0 z-[100]">
      ...
      <div className="flex items-center gap-3">
        ...
        <button onClick={() => setGuideOpen(true)} className="relative min-h-11 min-w-11 p-2.5 rounded-full hover:bg-primary-surface transition-colors" aria-label="Open user guide"><BookOpen size={18} className="text-body" /></button>
      ...
      {guideOpen && <div className="fixed inset-0 z-[90] flex items-center justify-center p-4" role="dialog" aria-modal="true"><div className="absolute inset-0 bg-ink/40" onClick={() => setGuideOpen(false)} /><div className="relative w-full max-w-2xl max-h-[85vh] overflow-y-auto rounded-2xl bg-canvas border border-hairline shadow-2xl p-6"><div className="flex items-start justify-between mb-5"><div><h2 className="text-lg font-bold text-ink">HRIS Demo Guide</h2><p className="text-xs text-muted mt-1">Panduan singkat penggunaan aplikasi.</p></div><button onClick={() => setGuideOpen(false)} aria-label="Close guide" className="min-h-10 min-w-10 rounded-lg hover:bg-surface-soft flex items-center justify-center"><X size={16} className="text-muted" /></button></div><div className="grid gap-4 sm:grid-cols-2 text-xs text-body"><div className="rounded-xl bg-surface-soft p-4"><h3 className="font-bold text-ink mb-2">Roles</h3><p><b>HR/Admin:</b> employee, approval, payroll, documents, recruitment.</p><p className="mt-2"><b>Employee:</b> profile, attendance, leave, and own documents.</p></div><div className="rounded-xl bg-surface-soft p-4"><h3 className="font-bold text-ink mb-2">Workflows</h3><p>Gunakan Employees, Attendance, Leave, Expenses, Documents, Onboarding, Recruitment, Notifications, dan Audit Log.</p><p className="mt-2">Refresh halaman setelah mutation untuk memastikan persistence.</p></div></div><p className="text-[11px] text-muted mt-5">Lihat dokumentasi lengkap di <code>docs/PANDUAN-PENGGUNAAN.md</code>. Ini adalah demo; payroll statutory, SSO/MFA, dan beberapa modul analytics belum enterprise-ready.</p></div></div>}
    </header>
  );
}
