      ...
      <div className="relative w-full max-w-2xl rounded-2xl bg-canvas border border-hairline shadow-2xl max-h-[90vh] overflow-y-auto">
        ...
        {report.type === 'payroll' && (
          <div className="px-6 py-5">
            ...
            <ResponsiveContainer width="100%" height={280}><LineChart data={data?.payroll ?? []} margin={{ top: 5, right: 20, bottom: 5, left: 0 }}><CartesianGrid strokeDasharray="3 3" stroke="var(--hairline-soft)" /><XAxis dataKey="month" tick={{ fontSize: 11, fill: '#94a3b8' }} axisLine={false} tickLine={false} /><YAxis tick={{ fontSize: 11, fill: '#94a3b8' }} axisLine={false} tickLine={false} /><Tooltip contentStyle={{ fontSize: 12, borderRadius: 12, border: '1px solid var(--hairline)', backgroundColor: 'var(--canvas)', color: 'var(--ink)' }} /><Legend iconType="circle" iconSize={8} wrapperStyle={{ fontSize: 11 }} /><Line type="monotone" dataKey="gross" stroke="#6366F1" strokeWidth={2} dot={{ r: 4 }} name="Gross" /><Line type="monotone" dataKey="net" stroke="#10B981" strokeWidth={2} dot={{ r: 4 }} name="Net" /><Line type="monotone" dataKey="tax" stroke="#F59E0B" strokeWidth={2} dot={{ r: 4 }} name="Tax" /></LineChart></ResponsiveContainer>
          </div>
        )}
        ...
        {report.type === 'attendance' && (
          <div className="px-6 py-5">
            ...
            <ResponsiveContainer width="100%" height={280}><LineChart data={data?.attendance ?? []} margin={{ top: 5, right: 20, bottom: 5, left: 0 }}><CartesianGrid strokeDasharray="3 3" stroke="var(--hairline-soft)" /><XAxis dataKey="month" tick={{ fontSize: 11, fill: '#94a3b8' }} axisLine={false} tickLine={false} /><YAxis domain={[0, 100]} tick={{ fontSize: 11, fill: '#94a3b8' }} axisLine={false} tickLine={false} /><Tooltip contentStyle={{ fontSize: 12, borderRadius: 12, border: '1px solid var(--hairline)', backgroundColor: 'var(--canvas)', color: 'var(--ink)' }} /><Legend iconType="circle" iconSize={8} wrapperStyle={{ fontSize: 11 }} /><Line type="monotone" dataKey="present" stroke="#10B981" strokeWidth={2} dot={{ r: 4 }} name="Present %" /><Line type="monotone" dataKey="late" stroke="#F59E0B" strokeWidth={2} dot={{ r: 4 }} name="Late %" /><Line type="monotone" dataKey="absent" stroke="#EC4899" strokeWidth={2} dot={{ r: 4 }} name="Absent %" /></LineChart></ResponsiveContainer>
          </div>
        )}
