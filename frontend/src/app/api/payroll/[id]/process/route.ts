...
export async function POST(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const { error: deleteError } = await client.from('payroll_entries').delete().eq('organization_id', user.organization_id).eq('period_id', params.id); if (deleteError) throw deleteError;
    if (rows.length) { const { error } = await client.from('payroll_entries').insert(rows); if (error) throw error; }
    const { data: updated, error: updateError } = await client.from('payroll_periods').update({ status: 'processed', processed_at: new Date().toISOString(), updated_at: new Date().toISOString() }).eq('organization_id', user.organization_id).eq('id', params.id).select('*').single();
