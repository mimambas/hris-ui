...
export async function POST(request: Request) {
  try {
    ...
    const { data: user } = await client.from('users').select('id,role,is_active').eq('id', payload.sub).maybeSingle();
    if (!user?.is_active) return NextResponse.json({ detail: 'User not found' }, { status: 401 });
    const access = await new SignJWT({ sub: user.id, role: user.role, type: 'access' }).setProtectedHeader({ alg: 'HS256' }).setExpirationTime('15m').sign(new TextEncoder().encode(secretValue));
